...
처음 저장이 일어나는 시점에 자동 생성됩니다(파일이 없다고 오류를 내지 않다). 이후
프로그램을 재실행해도 같은 `--data-dir`을 가리키는 한 이전에 저장한 거래/카테고리/예산이
그대로 유지됩니다 — 별도의 초기화나 마이그레이션 절차가 필요 없다.
...
예산 금액은 항상 양수만 저장됩니다("무제한 예산"이라는 개념 자체가 없다). `summarize()`의
`(total_expense / budget.amount * 100) if budget.amount else 0`은 0으로 나누기를 막는 방어
코드일 뿐, 실제 CLI로는 도달할 일이 없다. 예산 관리를 아예 안 하려면 `budget set` 자체를
...
- 정상 종료 시 exit code `0`, `AppError` 발생 시 `1`, `Ctrl+C`(`KeyboardInterrupt`)로 입력을
  ...
  `handle_errors`).
- 셸에서 종료 코드는 명령 실행 직후 `echo $?`로 확인할 수 있다.
  ```bash
  $ python -m budget_app delete --id TX-999999
  [오류] id=TX-999999 거래를 찾을 수 없다.
...
책임 경계를 명시적으로 나눴다 — 예를 들어 카테고리 존재 검증은 `CategoryService.exists`가
...
*args, **kwargs로 쓰면 데코레이터가 원본 함수의 인자 개수/이름을 몰라도 그대로 다 받아서 그대로 다 넘겨줄 수 있다. 그래서 `@handle_errors`를 인자 없는 `main()`에도, 인자 1개인 `run_add(args)`에도, 인자 여러 개인 다른 함수에도 똑같은 코드로 재사용할 수 있다.
...
개선 구현(카운터 파일, 월별 파일 분할 등)은 하지 않았다.
...
# 동료평가 시 나눈 이야기들
### 삭제된 거래번호는 공석으로 남는가?
이 질문을 받았을 때 자동으로 채번하는것은 구현했는데, 거래가 삭제된 이후 번호가 공석이될지는 의문이었다. 그래서 평가 시 같이 확인해보았다.

[확인]
- 거래를 1~4까지 생성한다.
- 거래 2와 4를 삭제 한다.
- 거래 신규 생성 시 5번이 아닌, 삭제됐던 4의 번호를 재사용해 4로 채번되는것을 확인했다.

[원인]
- 기존 `next_id()`는 `transactions.jsonl`에 남아있는 거래의 최댓값 + 1 로 채번된다.
- 그 외 별도의 재사용 방지 로직이 없었다.

[개선]
- `transactions.seq`에 마지막으로 발급된 번호를 별도로 영속시켜 `add` / `import_batch` 가 이 카운터만 증가시키도록 변경한다.
- 삭제는 카운터에 영향을 주지 않으므로, 삭제된 번호는 다시 발급되지 않는다.
- 카운터 파일이 없는 기존 데이터는 최초 1회 `transactions.jsonl`의 최댓값으로 초기화되어 하위 호환된다.

[검증]
- 거래 1, 2 생성
- 거래 2 삭제
- 거래 신규 생성 시 3번으로 자동 채번됨 (삭제번호 재사용되지 않는것을 확인)
...
[저장 완료] id=TX-000003
```
- transactions.seq : 마지막으로 채번된 id 3번을 파일에 영속 처리

    ![alt text](docs/screenshots/transactions_seq.png)

<br>
2. CSV import 가져오기와 데이터 정합성 처리 방법(전체성공 또는 전체실패, 부분성공, 롤백, 결과리포트)
CSV 가져오기에는 다음과 같은 처리 정책이 있다.

- 전체 성공 또는 전체 실패
    모든 행을 먼저 검증한 뒤, 전부 정상일 때만 저장한다. 한 행이라도 실패하면 아무것도 저장하지 않는다. 데이터 간 연관성이 강하거나 일부만 반영되면 안 되는 업무에 적합하다.

- 부분 성공
    행마다 검증하고 정상 행은 저장하며, 잘못된 행만 건너뛴다. 서로 독립적인 거래 데이터를 대량으로 가져올 때 유용하다.
    가계부 거래는 일반적으로 각 행이 서로 독립적이고 일부 오류때문에 정상 데이터까지 버리지 않게 하기위해, 과제에서는 이 방법을 이용해 `import_csv()`에서 각 행마다 `service.add()`를 호출하고, `ValidationError`가 발생한 행은 건너뛰면서 `imported`, `skipped`를 집계한다.

- 롤백
    저장 도중 오류가 발생하면 이번 가져오기에서 저장한 내용을 모두 이전 상태로 되돌린다. DB에서는 트랜잭션으로 처리하기 쉽지만, 파일 저장 방식에서는 원본 백업이나 임시 파일을 이용해 직접 구현해야 한다.

- 결과 리포트
    성공·실패 정책과 별개의 기능이다. 전체 건수, 성공 건수, 실패 건수뿐 아니라 실패한 행 번호와 실패 원인을 보여주거나 별도 파일로 저장한다.

추후 아래와 같은 구현을 보완할 수 있다.
- 실패 건수뿐 아니라 행 번호, 입력값, 실패 이유를 기록한다.
- 헤더 누락으로 발생하는 KeyError, 파일 열기 실패, 잘못된 CSV 형식도 사용자용 예외로 변환한다.
- 정책 이름을 partial처럼 명시하거나, 필요하면 all-or-nothing 모드도 선택할 수 있게 한다.
- 부분 성공이 요구사항에 맞는지 README에 설계 이유를 기록한다.

<br>
3. Python의 `None`과 Java의 `void`·`Void`
평가 중 타입 힌트에서 반환 타입을 `None`으로 작성한 의미에 관한 질문을 받았다. 나는 -> `None`은 함수가 의미 있는 값을 반환하지 않으며, 실행 결과로 `None`을 반환한다는 의미라고 답했다. 이 과정에서 Java의 `void`와 비슷하다는 생각이 들었지만, 두 개념은 완전히 같지는 않다.

Python의 `None`은 `None`Type에 속하는 실제 값이다. Python 함수는 return문을 생략하거나 값 없이 return을 사용해도 자동으로 `None`을 반환한다.
...
위 함수에는 return `None`이 없지만, 호출 결과는 `None`이다. 
따라서 -> `None`은 “이 함수의 반환값을 다른 계산에 사용하지 않는다”는 의도를 나타내는 타입 힌트이다. 
return `None`은 생략할 수 있으며, return과 return `None`도 같은 결과를 낸다.

<br>
반면 Java의 `void`는 실제 값이나 객체가 아니라, 메서드가 반환값을 제공하지 않는다는 문법적 표시이다.
...
Java의 `Void`는 `void`와도 다르다. `Void`는 제네릭처럼 타입을 반드시 지정해야 하는 상황에서 반환값이 없음을 표현하기 위해 사용하는 참조 타입이며, 일반적으로 가질 수 있는 값은 null뿐이다.

정리하면 Python의 -> `None`과 Java의 `void`는 “호출자가 사용할 의미 있는 반환값이 없다”는 의도는 비슷하다. 하지만 Python의 `None`은 실제로 반환되는 값이고, Java의 `void`는 반환값 자체가 없다는 차이가 있다.