...
### 2. CSV import 가져오기와 데이터 정합성 처리 방법
...
- 정책 이름을 `partial`처럼 명시하거나, 필요하면 `all-or-nothing` 모드도 선택할 수 있게 한다.
...
평가 중 타입 힌트에서 반환 타입을 `None`으로 작성한 의미에 관한 질문을 받았다. 

나는 -> `None`은 함수가 의미 있는 값을 반환하지 않으며, 실행 결과로 `None`을 반환한다는 의미라고 답했다. 이 과정에서 Java의 `void`와 비슷하다는 생각이 들었지만, 두 개념은 완전히 같지는 않다.

<br>
Python의 `None`은 `None`Type에 속하는 실제 값이다. 

Python 함수는 return문을 생략하거나 값 없이 return을 사용해도 자동으로 `None`을 반환한다.
